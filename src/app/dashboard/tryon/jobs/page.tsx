...
function pipelineTrace(item: GenerationModuleExecution): PipelineTraceEvent[] {
  const value = item.runtime_metrics?.pipeline_trace;
  if (!Array.isArray(value)) return [];
  return value.filter((entry): entry is PipelineTraceEvent => Boolean(entry && typeof entry === "object"));
}

function pipelineTraceTitle(event: PipelineTraceEvent) {
  if (event.kind === "workflow_bindings") return "Bindings enviados a ComfyUI";
  if (event.kind === "python_inputs") return "Entradas resueltas del paso Python";
  if (event.kind === "python_outputs") return "Salidas reales del paso Python";
  return event.kind || "Evento de pipeline";
}
...
function bindingResources(binding: Record<string, unknown>, inputs: unknown) {
  const explicit = dedupeResources(resultResources(binding.source_asset, "Asset enviado"));
  if (explicit.length) return explicit;
  return dedupeResources(resultResources(valueAtPath(inputs, binding.source_path), "Asset enviado"));
}
...
export default function UnifiedAiJobsPage() {
  ...
  const buildExecutionParams = useCallback((targetModuleId: number, targetPage: number) => {
    const params = new URLSearchParams({
      ...
      skip: String(targetPage * pageSize),
    });
    ...
    return params;
  }, [dateFrom, dateTo, debouncedSearch, engine, pageSize, status, userId]);
  ...
  useEffect(() => { modulePagesRef.current = {}; setModulePages({}); }, [dateFrom, dateTo, debouncedSearch, engine, moduleId, pageSize, status, userId]);
    ...
    {isLoading ? <section className="luxia-panel flex min-h-72 items-center justify-center rounded-3xl"><div className="text-center"><LoaderCircle className="mx-auto animate-spin text-red-500"/><p className="mt-4 text-sm text-zinc-500">Consultando trabajos...</p></div></section> : grouped.length === 0 ? <section className="luxia-panel rounded-3xl p-12 text-center"><Activity className="mx-auto text-zinc-700"/><h2 className="mt-4 font-semibold text-white">No hay trabajos que coincidan</h2><p className="mt-2 text-sm text-zinc-500">Ajusta los filtros o ejecuta un módulo de generación.</p></section> : <div className="space-y-4">
      {grouped.map(([id, jobs]) => {
        ...
        return <section key={id} className="luxia-panel overflow-hidden rounded-3xl">
          ...
          {isExpanded && <div className="flex flex-col gap-3 border-t border-white/6 p-4 lg:flex-row lg:items-center lg:justify-between">
            <p className="text-xs text-zinc-500">Página {(modulePages[id] ?? 0) + 1} de {Math.max(1, Math.ceil((moduleTotals[id] ?? 0) / pageSize))} · {(moduleTotals[id] ?? 0).toLocaleString("es-MX")} trabajos de este módulo</p>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <label className="flex items-center gap-2 text-xs text-zinc-500">Mostrar
                <select value={pageSize} onChange={(event)=>changePageSize(Number(event.target.value))} disabled={isLoading} className="h-10 rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-zinc-300 outline-none disabled:opacity-40">
              ...
              <button type="button" disabled={((modulePages[id] ?? 0) + 1) * pageSize >= (moduleTotals[id] ?? 0) || isLoading} onClick={() => void changeModulePage(id, (modulePages[id] ?? 0) + 1)} className="h-10 rounded-xl border border-white/10 px-4 text-sm text-zinc-300 disabled:opacity-40">Siguiente</button>
              </div>
            </div>
          </div>}
        </section>
