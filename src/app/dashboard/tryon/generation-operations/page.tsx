...
type Execution = { id:string; module_key:string; engine:string; status:string; progress:number; created_at:string; error?:string|null; user_id?:number|null };
...
export default function GenerationOperationsPage(){
 ...
 return <div className="space-y-7">
  ...
  <section className="luxia-panel overflow-hidden rounded-3xl"><div className="flex items-center justify-between border-b border-white/10 p-5"><h2 className="font-semibold text-white">Monitor de ejecuciones</h2><button onClick={()=>void load()} className="text-xs text-zinc-400 hover:text-white">Actualizar</button></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="text-xs uppercase text-zinc-600"><tr><th className="p-4">Módulo</th><th>Motor</th><th>Origen</th><th>Estado</th><th>Progreso</th><th>Creado</th><th>Error</th></tr></thead><tbody>{executions.map(e=><tr key={e.id} className="border-t border-white/5"><td className="p-4"><p className="text-white">{e.module_key}</p><p className="font-mono text-[10px] text-zinc-700">{e.id}</p></td><td className="text-zinc-400">{e.engine}</td><td className="text-zinc-500">{e.user_id?`Usuario #${e.user_id}`:"Admin"}</td><td className="uppercase text-zinc-400">{e.status}</td><td className="pr-4"><div className="h-2 w-32 overflow-hidden rounded-full bg-white/5"><div className="h-full bg-red-600" style={{width:`${e.progress}%`}}/></div></td><td className="text-zinc-500">{new Date(e.created_at).toLocaleString()}</td><td className="max-w-xs truncate text-red-300">{e.error??"—"}</td></tr>)}</tbody></table></div></section>
 </div>
}
