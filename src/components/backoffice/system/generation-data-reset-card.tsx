...
type Result = { success: boolean; deleted_storage_files: number; zeroed_users: number; stripe_subscriptions_cancelled: number; deleted: Record<string, number>; };

const LABELS: Record<string, string> = {
  generation_module_executions: "Ejecuciones dinámicas", tryon_jobs: "Trabajos TryOn", generation_financial_records: "Registros financieros",
  ...
  background_jobs: "Trabajos en segundo plano", storage_files: "Archivos relacionados", tokens_to_zero: "Tokens que se pondrán en cero",
};
...
export function GenerationDataResetCard() {
  ...
  const [deleteFiles, setDeleteFiles] = useState(true); const [cancelStripe, setCancelStripe] = useState(false);
  ...
  const execute = async () => { if (!preview || confirmation !== preview.confirmation_text) return; setExecuting(true);
    try { const r=await browserApiRequest<Result>("/api/admin/maintenance/generation-reset", {method:"POST", body:JSON.stringify({confirmation,delete_storage_files:deleteFiles,cancel_stripe_subscriptions:cancelStripe})});
      toast.success(`Actividad reiniciada: ${r.zeroed_users} saldos en cero, ${r.deleted_storage_files} archivos eliminados.`); setConfirmation(""); await loadPreview(); }
  ...
  return <section className="luxia-panel mt-5 overflow-hidden rounded-3xl border border-red-500/15">
    <div className="border-b border-red-500/10 bg-red-950/10 p-6"><div className="flex items-start gap-4"><div className="flex size-12 shrink-0 items-center justify-center rounded-2xl border border-red-500/20 bg-red-950/30 text-red-400"><DatabaseZap size={22}/></div><div>
      ...
      <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-500">Elimina generaciones, tokens, compras, pagos, suscripciones locales, finanzas y archivos relacionados. Conserva usuarios, módulos, pricing, planes, paquetes, cupones, proveedores y configuración.</p></div></div></div>
