...
export function PricingSimulatorCard() {
  ...
  return <section className="luxia-panel mt-5 rounded-3xl p-6">
    ...
    <form onSubmit={simulate} className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <Field label="Valor base de 1 token (USD)"><input className="h-11 w-full rounded-xl border border-white/8 bg-[#09090a] px-3 text-sm text-white outline-none focus:border-sky-500/30" value={tokenValue} onChange={(e)=>setTokenValue(e.target.value)}/></Field>
