...
export function DynamicGenerationForm({ inputs, values, onChange, disabled }: Props) {
  ...
  return <div className="space-y-4"><p className="rounded-xl border border-emerald-500/15 bg-emerald-500/5 px-3 py-2 text-xs text-emerald-300">Las imágenes y archivos se cargarán automáticamente al entorno del motor; no escribas rutas locales de Windows ni Docker.</p><div className="grid gap-4 md:grid-cols-2">
    {[...inputs].sort((a,b)=>a.position-b.position).map((input) => {
      ...
      const isMultilineText = input.input_type === "text" && (rules.multiline === true || Number(rules.rows ?? 1) > 1);
      const common = { disabled, required: input.is_required, className: "gm-input" };
      return <label key={input.key} className={isMultilineText || input.input_type === "json" || input.input_type === "image" || input.input_type === "file" ? "md:col-span-2" : ""}>
        ...
        {input.input_type === "boolean" ? <input type="checkbox" checked={Boolean(values[input.key])} onChange={e=>setValue(input.key,e.target.checked)} disabled={disabled} className="h-5 w-5 accent-red-600"/> :
         isSelect ? <select {...common} value={String(values[input.key] ?? "")} onChange={e=>setValue(input.key,e.target.value)}><option value="">Selecciona una opción</option>{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select> :
         isMultilineText ? <textarea {...common} rows={Number(rules.rows ?? 5)} value={String(values[input.key] ?? "")} onChange={e=>setValue(input.key,e.target.value)}/> :
