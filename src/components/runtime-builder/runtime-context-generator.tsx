...
export function RuntimeContextGenerator() {
  ...
  const exportVolume = async () => {
    ...
    const payload: RuntimeModelExportSettings = {
      ...
      docker_path: (modelDestination === "docker_volume" || modelDestination === "modal") ? dockerPath.trim().replace(/^\/+|\/+$/g, "") : "",
    ...
    try {
      ...
      setVolumeResult(response); toast.success(modelDestination === "docker_volume" ? `Modelos exportados al volumen ${dockerVolume}.` : modelDestination === "modal" ? "Modelos exportados a Modal." : "Modelos exportados a la carpeta local.");
  ...
  return <div className="space-y-5">
    ...
    <section className="luxia-panel rounded-3xl p-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Field label="Destino de exportación"><select className={inputClass} value={modelDestination} onChange={e=>setModelDestination(e.target.value as RuntimeModelExportSettings["destination_type"])}><option value="local">Carpeta local</option><option value="docker_volume">Volumen Docker</option><option value="modal">Modal</option></select></Field>
